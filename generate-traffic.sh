...
while true; do
    ...
    curl -s -o /dev/null "https://api.animalbuttons.biz" 

    # Call snail api with a 50% chance
    random_number=$((RANDOM % 2))
    # If the random number is 0, execute the code
    if [ $random_number -eq 0 ]; then
        curl -s -o /dev/null "https://api.animalbuttons.biz/snail" 
    fi
    ...
    if [ $random_number -le 2 ]; then
        curl -s -o /dev/null "https://api.animalbuttons.biz/rabbit" 
    fi
    ...
    if [ $random_number -le 9 ]; then
        curl -s -o /dev/null "https://api.animalbuttons.biz/panda" 
    fi
    ...
    # If the random number is 0, execute the code
    if [ $random_number -eq 0 ]; then
        curl -s -o /dev/null "https://api.animalbuttons.biz/beaver" 
    fi
done